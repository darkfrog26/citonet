...
import com.outr.citonet.{ArrayBufferPool, Method, Protocol, URL}
...
class ProxyConnection(server: ProxyServer, socket: Socket, protocol: Protocol) extends Logging {
  ...
  private val originBuffer = ArrayBufferPool()
  private val proxyBuffer = ArrayBufferPool()
  ...
  def close() = if (!closed) {
    ...
    ArrayBufferPool.release(originBuffer)
    ArrayBufferPool.release(proxyBuffer)
